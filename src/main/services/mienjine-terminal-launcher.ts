...
const wrapCommand = (commandText: string): string =>
  `${commandText}; exit_code=$?; printf "\\nMienjine exited with code %s. Press Enter to close..." "$exit_code"; read _; exit "$exit_code"`;
...
export class MienjineTerminalLauncher {
  launch(cwd: string, startScriptPath: string): string {
    ...
    if (platform() === 'win32') {
      return this.launchInWindowsTerminal(cwd, startScriptPath, launchCommand.label);
    }
  ...
  private launchDefaultTerminal(cwd: string, commandText: string): void {
    const scriptPath = join(cwd, 'run-mienjine.command');
    const script = [
      '#!/bin/sh',
      `cd ${shellQuote(cwd)}`,
      commandText,
      'exit_code=$?',
      'printf "\\nMienjine exited with code %s. Press Enter to close..." "$exit_code"',
      'read _',
      'exit "$exit_code"',
      '',
    ].join('\n');
  ...
  private launchInWindowsTerminal(cwd: string, startScriptPath: string, sandboxLabel: string): string {
    const scriptPath = join(cwd, 'run-mienjine.cmd');
    writeFileSync(scriptPath, [
      '@echo off',
      'cd /d "%~dp0"',
      `call ${quoteWindowsCmdArg(startScriptPath)}`,
      '',
    ].join('\r\n'), { mode: 0o600 });
...
export const mienjineTerminalLauncher = new MienjineTerminalLauncher();
